...
export function refresh(route=state.route) {
  return renderComponent(route.component, route.title).then(function() {
    return route;
  });
}
...
export function pushRoutes(path, headers={}) {
  _getRouteObject(path,  headers).then(function(route) {
    state.route = route;
  });
}
