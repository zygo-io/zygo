...
export function route(path, headers={}) {
  let matches = Routes.match(path, routes);
  let matchIndex = 0;
  if (!matches[matchIndex]) throw new Error("No match found for path: " + path);
  ...
  return _route(matches, matchIndex, context);
}

//Helper function for multiple match routing.
function _route(matches, matchIndex, context) {
    ...
    .catch((error) => {
      ...
      if (error instanceof Routes.RouteRedirect) {
        if (error.redirect === false) {
          matchIndex++;
          if (matches[matchIndex]) return _route(matches, matchIndex, context);
          return this.route('default', context.headers);
        }
        ...
        if (error.redirect.status) {
          ...
          if (matches[matches.length - 2] && matches[matches.length - 2]._isDefault)
            return _route(matches, matches.length - 2, context);
          return _route(matches, matches.length - 1, context);
        }

        //A normal route redirect, head there.
        return route(error.redirect, context.headers);
      }
      throw error;
    });
}
...
export function setMetadata() {
  return new Promise((resolve, reject) => {
    document.getElementsByTagName('title')[0].innerHTML = context.pageTitle || '';
