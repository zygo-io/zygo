...
export function refresh() {
  ...
  return Render.renderRoutes(context.loadingRequest.routes, context)
    .then(() => {
      //Swap back. We need to ensure props are _exactly_ the same when we render.
      //Otherwise this rapidly becomes unperformant in extreme cases.
      context.currentRequest = context.loadingRequest;
      context.loadingRequest = undefined;
    });
}
...
export function setMetadata() {
  return new Promise((resolve, reject) => {
    ...
    //There may be no loading request if we are setting metdata at initial render from server.
    if (context.loadingRequest) {
      context.currentRequest = context.loadingRequest;
      context.loadingRequest = undefined;
    }
